...
class BillExtrasTableViewAdapter(private val context: Context?) :

    AbstractTableAdapter<ColumnHeaderModel?, RowHeaderModel?, CellModel?>() {

    private val extrasTableViewModel: BillExtrasTableViewModel = BillExtrasTableViewModel()
    ...
    fun setExtrasList(extras: List<BillExtra>, currency: String) {
        extrasTableViewModel.generateListForTableView(extras, currency)
        setAllItems(
            extrasTableViewModel.columnHeaderModeList, extrasTableViewModel
                .rowHeaderModelList, extrasTableViewModel.cellModelList
        )
    }
}